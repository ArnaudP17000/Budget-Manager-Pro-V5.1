...
class BonCommandeView(QWidget):
    ...
    def _creer_projet_depuis_bc(self, bc_id, data, dlg):
        ...
        try:
            ...
            statut_projet = bc_to_statut.get(statut_bc, 'EN_ATTENTE')
            ...
            projet_data = {
                ...
                'phase':         'REALISATION' if statut_projet == 'ACTIF' else 'ETUDE',
                'priorite':      'MOYENNE',
                ...
                'avancement':    0,
                # entite_id non disponible dans la table projets
            }
    ...
    def _proposer_tache_bc(self, bc):
        """Propose de créer une tâche après imputation d'un BC."""
        rep = QMessageBox.question(
            self, "Créer une tâche ?",
            f"Voulez-vous créer une tâche de suivi pour ce BC ?\n"
            f"({bc.get('objet','')[:60]})",
        ...
        try:
            ...
            tache_init = {
                'titre':       f"Suivi BC {bc.get('numero_bc','')} — {bc.get('objet','')[:50]}",
                'description': (f"Fournisseur : {bc.get('fournisseur_nom','')}\n"
                                f"Montant TTC : {bc.get('montant_ttc',0):,.2f} €\n"
                                f"Objet : {bc.get('objet','')}"),
                'statut':      'EN_COURS',
                'priorite':    'MOYENNE',
                'projet_id':   bc.get('projet_id'),
            }
            dlg = TacheDialog(self, tache=tache_init, projet_obligatoire=False)
            dlg.exec_()
        except Exception as e:
