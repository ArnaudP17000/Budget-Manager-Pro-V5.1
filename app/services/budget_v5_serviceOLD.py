...
class BudgetV5Service:
    ...
    def get_synthese_budgets(self, exercice=None):
        """Synthèse tous budgets pour le dashboard."""
        where = "WHERE exercice=?" if exercice else ""
        params = (exercice,) if exercice else None
        rows = self.db.fetch_all(f"""
            SELECT * FROM v_synthese_budget
            {where}
    ...
    def create_ligne(self, data):
        data['date_creation'] = datetime.now().isoformat()
        data['date_maj'] = data['date_creation']
        # montant_solde = montant_vote (rien d'engagé au départ)
    ...
    def imputer_bc_sur_ligne(self, bc_id, ligne_id):
        """
        ...
        try:
            ...
            solde   = float(ligne['montant_solde'] or 0)
